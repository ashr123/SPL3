...
public class Test
{
	...
	private static int numOfClients=50;
	...
	private static void normalClient(String client) throws IOException
	{
		     ...
		     BufferedWriter out=new BufferedWriter(new OutputStreamWriter(sock.getOutputStream())))
		{
			...
			for (String cmd : cmds)
			{
				sleep((int)(Math.random()*Test.randomMultConstant));
				out.write(cmd);
				out.flush();
	...
	private static void clientReturn(String client) throws IOException
	{
		     ...
		     BufferedWriter out=new BufferedWriter(new OutputStreamWriter(sock.getOutputStream())))
		{
			...
			for (String cmd : cmds)
			{
				sleep((long)(Math.random()*Test.randomMultConstant));
				out.write(cmd);
				out.flush();
				String line=in.readLine();
				System.out.println(client+": "+line);
